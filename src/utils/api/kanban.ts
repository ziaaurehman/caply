...
export const kanbanAPI = {
  ...
  getBoards: async (projectId: string): Promise<BoardsResponse> => {
    const response = await fetch(`/api/kanban/boards?project_id=${projectId}`);
    if (!response.ok) {
  ...
  // Get lists by board
  getLists: async (boardId: string): Promise<ListsResponse> => {
    const response = await fetch(`/api/kanban/lists?board_id=${boardId}`);
    if (!response.ok) {
      const errorData = await response.json();
  ...
  // Create list
  createList: async (data: CreateListData): Promise<ListResponse> => {
