...
import { type CapacityOverview, type ResourceAllocation, capacityAPI } from "@/utils/api/capacity"
...
export default function WeeklyCapacityTable({
  ...
  const [memberTasks, setMemberTasks] = useState<
    Record<string, Record<string, { tasks_count: number; estimated_hours: number }>>
  >({})
  ...
  const toggleMemberExpansion = (memberId: string) => {
    ...
    if (newExpanded.has(memberId)) {
      ...
      if (organizationId && projectIds.length > 0) {
        capacityAPI
          .getTasksSummary(organizationId, { user_id: memberId, project_ids: projectIds, include_tasks: false })
          .then((res) => {
            const map: Record<string, { tasks_count: number; estimated_hours: number }> = {}
            res.summary.forEach((s) => {
              map[s.project_id] = { tasks_count: s.tasks_count, estimated_hours: s.estimated_hours }
            })
      ...
      <table className="min-w-full bg-gray-50">
        <thead>
          <tr className="border-b border-gray-200 bg-gray-50">
            <th className="text-left px-6 py-4 text-xs font-medium text-gray-500 uppercase tracking-wider">RESOURCE</th>
                ...
                <tr className="bg-gray-50">
                    ...
                    <div className="flex items-center space-x-3">
                      <div className="flex-1">
                        <div className="flex items-center justify-between text-sm">
                          ...
                          {statusLabel && <span className="text-xs text-red-600">{statusLabel}</span>}
                        </div>
                        {/* Progress Bar */}
                  ...
                  {weeksData.map((week) => {
                    ...
                    const capacityForCell =
                      week.type === "day" ? Math.round((member.capacity / 5) * 10) / 10 : member.capacity
                    ...
                    return (
                      <td
                        ...
                        onClick={() =>
                          onWeekCellClick?.({ userId: member.user.id, week, member, allocations: member.allocations })
                        }
                      >
                        {isWeekend ? (
                          ...
                          {week.type === "day" ? (
                            <>
                              <div className="font-semibold text-gray-900 mb-2">
                                {week.weekNumber} • {new Date(week.startDate).toLocaleDateString()}
                              </div>
                              {isWeekend ? (
                                <div className="text-gray-600 italic">Weekend - No work scheduled</div>
                              ) : (
                                <div className="space-y-1">
                                  <div className="flex justify-between items-center">
                                    <span className="text-gray-600">Allocated:</span>
                                    <span className="font-medium text-gray-900">{allocationForCell}h</span>
                                  </div>
                                  <div className="flex justify-between items-center">
                                    <span className="text-gray-600">Capacity:</span>
                                    <span className="font-medium text-gray-900">{capacityForCell}h</span>
                                  </div>
                                  <div className="flex justify-between items-center border-t border-gray-200 pt-1">
                                    <span className="text-gray-600">Available:</span>
                                    <span className="font-medium text-green-600">{availableForCell}h</span>
                                  </div>
                                </div>
                              )}
                            </>
                            ...
                            <>
                              <div className="font-semibold text-gray-900 mb-2">Week {week.weekNumber}</div>
                              <div className="space-y-1">
                                <div className="flex justify-between items-center">
                                  <span className="text-gray-600">Allocated:</span>
                                  <span className="font-medium text-gray-900">{allocationForCell}h</span>
                      ...
                      <td className="px-6 py-3 pl-16">
                        <div className="flex items-center">
                          ...
                          <button
                            ...
                            onClick={() => onProjectClick?.(allocation.project_id)}
                          >
                            ...
                            {isWeekend ? (
                              ...
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  setEditingHours(String(allocation.hours_per_week))
                                }}
                              >
                                {week.type === "day"
                    ...
                    <td className="px-6 py-3 pl-16">
                      <button
                        ...
                        onClick={() => {
                          ...
                          setAddProjectMemberName(member?.user?.full_name || "Member")
                        }}
                      >
