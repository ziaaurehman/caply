...
import { ChevronDown, ChevronRight, Filter, Download, ChevronLeft, X, Plus } from 'lucide-react';
...
export default function CapacityPlanningPage() {
  ...
  const { projects, assignments, fetchProjects, fetchAssignments, updateAssignment, deleteAssignment } = useProjectStore();
  ...
  const filteredEmployees = employees.filter(employee => {
    ...
    if (showOverallocatedOnly) {
      ...
      return allocation && allocation.total > employee.capacityHours;
    }
    return true;
  });
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredEmployees.map(employee => {
                  const allocation = weeklyAllocations[employee.id] || { total: 0, projects: [] };
