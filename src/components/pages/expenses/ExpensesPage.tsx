...
import { useExpenseStore, categories, currencies, Expense } from '@/lib/stores/expenseStore';
...
export default function ExpensesPage() {
  const { expenses, addExpense, updateExpense, deleteExpense, updateExpenseStatus } = useExpenseStore();
  ...
  const handleSubmit = (expense: Expense | Omit<Expense, 'id' | 'status'>) => {
    if ('id' in expense) {
      updateExpense(expense as Expense);
    } else {
      addExpense(expense as Omit<Expense, 'id' | 'status'>);
    }
    setShowModal(false);
    setSelectedExpense(null);
  };
                            ...
                            <td colSpan={8} className="px-6 py-4 bg-gray-50">
                              <div className="space-y-4">
                                ...
                                <div className="flex space-x-2">
                                  <Button 
                                    variant={expense.status === 'approved' ? 'outline' : 'default'}
                                    size="sm"
                                  ...
                                  <Button 
                                    variant={expense.status === 'rejected' ? 'outline' : 'default'}
                                    size="sm"
                                    onClick={() => updateExpenseStatus(expense.id, 'rejected')}
                                    disabled={expense.status === 'rejected'}
                                    className={expense.status !== 'rejected' ? "bg-red-600 text-white hover:bg-red-700" : ""}
                                  >
                                    Reject
                                  </Button>
                                  <Button 
