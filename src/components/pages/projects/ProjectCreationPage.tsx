...
import {
  projectAPI as projectAPIFromIndex,
  clientAPI,
  teamAPI,
...
export default function ProjectCreationPage() {
  ...
  const onSubmit = async (data: ProjectFormData) => {
    ...
    if (
      !data.selected_team_members ||
      data.selected_team_members.length === 0
    ) {
      const errorMessage =
      ...
      const payload = {
        ...
        billing_rate:
          data.project_type === "time_materials" && data.billing_rate
            ...
            : undefined,
        budget_hours:
          data.project_type === "time_materials" && data.budget_hours
            ? Number(data.budget_hours)
            : undefined,
        budget_amount:
          data.project_type === "fixed_fee" && data.budget_amount
  ...
  const handleRemoveCategory = (categoryToRemove: string) => {
    setTaskCategories(
      taskCategories.filter((category) => category !== categoryToRemove)
    );
  };
      ...
      <div className=" mx-auto bg-white rounded-lg shadow-md p-8">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
          ...
          <div className="p-6 border border-gray-200 rounded-lg">
            ...
            <div className="relative" id="team-dropdown">
              ...
              >
                <span className="block truncate">
                  {getSelectedTeamMembersCount(
                    watch("selected_team_members") || []
                  )}
                </span>
                  ...
                  <div className="p-2 space-y-1">
                    {loadingTeamMembers ? (
                      ...
                      teamMembers.map((member) => (
                        <label
                          key={member.id}
                          className="flex items-center px-2 py-1.5 hover:bg-orange-50 rounded cursor-pointer"
                        >
                          <input
                            type="checkbox"
                            value={member.id}
            ...
            {errors.selected_team_members && (
              <p className="mt-1 text-sm text-red-600">
                {errors.selected_team_members.message}
              </p>
            )}
          </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    {...register("billing_rate", {
                      ...
                      validate: {
                        validNumber: (value) => {
                          if (watchedProjectType !== "time_materials")
                            return true;
                          if (!value) return true; // Let required handle empty values
                          const num = Number(value);
                          if (isNaN(num)) return "Please enter a valid number";
                          if (num < 0)
                            return "Billing rate must be greater than or equal to 0";
                          if (num > 10000)
                            return "Billing rate must be less than $10,000/hour";
                          return true;
                        },
                      },
                    })}
                    ...
                    {...register("budget_hours", {
                      validate: {
                        validNumber: (value) => {
                          ...
                          if (isNaN(num)) return "Please enter a valid number";
                          if (num < 0)
                            return "Budget hours must be greater than or equal to 0";
                          if (num > 100000)
                            return "Budget hours must be less than 100,000";
                          return true;
                        },
                      },
                    })}
                  ...
                  {...register("budget_amount", {
                    ...
                    validate: {
                      validNumber: (value) => {
                        ...
                        if (num > 10000000)
                          return "Budget amount must be less than $10,000,000";
                        return true;
                      },
                    },
                  })}
          ...
          <div className="p-6 border border-gray-200 rounded-lg">
            ...
            <div className="space-y-4">
              <div className="flex items-start justify-between">
                <div>
                  <label
                    htmlFor="capacityPlanning"
                    className="block text-sm font-medium text-gray-700"
                  >
                    Capacity Planning
                  </label>
                  <p className="text-xs text-gray-500">
                    Link project with resource availability
                  </p>
                </div>
                <input
                  type="checkbox"
              ...
              <div className="flex items-start justify-between">
                <div>
                  <label
                    htmlFor="absenceIntegration"
                    className="block text-sm font-medium text-gray-700"
                  >
                    Absence Integration
                  </label>
                  <p className="text-xs text-gray-500">
                    View team availability in planning
                  </p>
                </div>
                <input
                  type="checkbox"
              ...
              <div className="flex items-start justify-between">
                <div>
                  <label
                    htmlFor="kanbanActivation"
                    className="block text-sm font-medium text-gray-700"
                  >
                    Kanban Activation
                  </label>
                  <p className="text-xs text-gray-500">
                    Enable Kanban board for this project
                  </p>
                </div>
                <input
                  type="checkbox"
              ...
              <div>
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <label
                      htmlFor="timesheetActivation"
                      className="block text-sm font-medium text-gray-700"
                    >
                      Timesheet Activation
                    </label>
                    <p className="text-xs text-gray-500">
                      Allow time tracking for this project
                    </p>
                  </div>
                  <input
                    type="checkbox"
          ...
          <div className="p-6 border border-gray-200 rounded-lg">
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center flex flex-col items-center justify-center h-48 mb-4">
              ...
              <label htmlFor="file-upload" className="cursor-pointer">
                <UploadCloud
                  className={`h-10 w-10 mb-3 ${isUploading ? "text-gray-300" : "text-gray-400"}`}
                />
                <p className="text-sm text-gray-600">
                  <span
                    className={`font-medium ${isUploading ? "text-gray-400" : "text-orange-600 hover:text-orange-500"}`}
                  >
            ...
            {uploadedDocuments.length > 0 && (
              <div className="space-y-2 mt-4">
                <h3 className="text-sm font-medium text-gray-700">
                  Existing Documents ({uploadedDocuments.length})
                </h3>
