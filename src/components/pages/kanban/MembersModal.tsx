...
export default function MembersModal({
  ...
  console.log("MembersModal props:", members);
  const [searchTerm, setSearchTerm] = useState("");
  const [localSelectedMembers, setLocalSelectedMembers] =
    useState<string[]>(selectedMembers);
  ...
  const handleMemberToggle = (memberId: string) => {
    ...
    if (isSelected) {
      setLocalSelectedMembers(
        localSelectedMembers.filter((id) => id !== memberId)
      );
    } else {
  ...
  const boardMembers = members.filter(
    (member) => !localSelectedMembers.includes(member.id)
  );
