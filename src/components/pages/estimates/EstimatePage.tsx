...
import { useEstimateStore, EstimateItem } from '@/lib/stores/estimateStore';

interface EstimatePageProps {
  editId?: string;
}

export default function EstimatePage({ editId }: EstimatePageProps) {
  const { estimates, clients, addEstimate, updateEstimate, addEstimateItem, updateEstimateItem, removeEstimateItem, updateEstimateStatus } = useEstimateStore();
  
  // Form state
  const [currentEstimateId, setCurrentEstimateId] = useState<string | null>(null);
  const [estimateNumber, setEstimateNumber] = useState('EST-' + Date.now());
  ...
  useEffect(() => {
    if (editId) {
      setCurrentEstimateId(editId);
    }
  }, [editId]);

  // Load estimate data when currentEstimateId changes
  useEffect(() => {
    if (currentEstimateId) {
      ...
      if (estimate) {
        ...
        setSelectedClient(estimate.clientId);
  ...
  const handleAddItem = () => {
    const newItem: Omit<EstimateItem, 'id' | 'order'> = {
      type: 'service',
      description: '',
      quantity: 1,
      unitPrice: 0,
    ...
    if (currentEstimateId) {
      addEstimateItem(currentEstimateId, newItem);
    } else {
  ...
  const handleRemoveItem = (id: string) => {
    if (currentEstimateId) {
      removeEstimateItem(currentEstimateId, id);
    } else {
  ...
  const updateItem = (id: string, updates: Partial<EstimateItem>) => {
    if (currentEstimateId) {
      updateEstimateItem(currentEstimateId, id, updates);
    } else {
  ...
  const handleSaveEstimate = () => {
    ...
    if (currentEstimateId) {
      updateEstimate(currentEstimateId, estimateData);
    } else {
  ...
  const handleSendEstimate = () => {
    if (currentEstimateId) {
      updateEstimateStatus(currentEstimateId, 'sent');
      setStatus('sent');
    } else {
      setStatus('sent');
      handleSaveEstimate();
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            {currentEstimateId ? 'Edit Estimate' : 'Create Estimate'}
          </h1>
