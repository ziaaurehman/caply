...
const InvitePage = () => {
  ...
  const validateInvitation = async () => {
    try {
      ...
      try {
        const response = await fetch('/api/debug?check=invitation&token=' + token)
        if (response.ok) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="bg-white p-8 rounded-lg shadow-lg max-w-md w-full mx-4">
          ...
          <p className="text-center text-gray-600 mt-4">Validating invitation...</p>
        </div>
      </div>
    )
  }
          ...
          <div className="text-center mb-6">
            ...
            <div className="flex items-center justify-center space-x-2 mb-6">
              ...
              <span className="text-sm text-gray-600">
                You'll be joining as a <strong>{invitation.role.display_name}</strong>
              </span>
            </div>

            {invitation.role.description && (
              <div className="bg-blue-50 rounded-lg p-4 mb-6 text-left">
                <h3 className="font-medium text-blue-900 mb-2">Role Details:</h3>
                <p className="text-blue-700 text-sm">{invitation.role.description}</p>
              </div>
            )}
          </div>
