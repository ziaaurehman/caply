  ...
  const loadProjects = useCallback(async () => {
    ...
    try {
      ...
      if (kanbanProjects.length > 0 && !selectedProjectId) {
        const savedProjectId = localStorage.getItem('kanban-selected-project')
        if (savedProjectId && kanbanProjects.find(p => p.id === savedProjectId)) {
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center max-w-md mx-auto">
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-6">
            <h3 className="text-lg font-semibold text-gray-800 mb-2">No Kanban Projects</h3>
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="relative" id="project-dropdown">
              ...
              >
                ...
                <input
                  type="text"
                  value={selectedProjectId ? getSelectedProjectName() : projectSearch}
                  onChange={(e) => {
                ...
                <ChevronDown className={`h-4 w-4 text-gray-700 ml-2 transition-transform ${isProjectDropdownOpen ? 'rotate-180' : ''}`} />
              </div>
                ...
                <div className="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
                  {filteredProjects.length > 0 ? (
                    ...
                    <div className="py-2 px-3 text-gray-500 text-sm">
                      {projectSearch ? 'No projects found' : 'No Kanban projects available'}
                    </div>
                  )}
                </div>
              )}
