...
export async function PATCH(
  ...
  if (typeof is_archived === 'boolean' && is_archived !== existingList.is_archived) {
    try {
      ...
      if (cardsError) {
        ...
        console.log(`🔄 ${is_archived ? 'Archived' : 'Unarchived'} ${updatedCards?.length || 0} cards in list: ${list.name}`);
      ...
      console.error('Error updating cards when archiving/unarchiving list:', cardsUpdateError);
    }
  }
  ...
  await supabase
    ...
    .insert([{
      user_id: userContext!.userId,
      board_id: existingList.board_id,
      action_type: is_archived ? 'archive' : (is_archived === false ? 'unarchive' : 'update'),
      entity_type: 'list',
      entity_id: listId,
      details: { 
        changes: body,
        cards_affected: is_archived !== undefined ? 'Cards ' + (is_archived ? 'archived' : 'unarchived') + ' with list' : undefined
      }
    }]);

  // *** PROPER CACHE INVALIDATION ***
  try {
    const cacheKey = `kanban:lists:${existingList.board_id}:${organizationId}`;
    await redisDel(cacheKey);
    console.log(`🔄 Invalidated lists cache for board: ${existingList.board_id}`);
  } catch (e) {
    console.warn('Failed to invalidate lists cache:', e);
  }
