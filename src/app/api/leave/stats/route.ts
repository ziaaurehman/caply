...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to fetch leave stats' }, { status: 500 })
    }
    ...
    const stats = {
      ...
      total_days_requested: requests?.reduce((sum, r) => sum + (r.days_requested || 0), 0) || 0,
      total_days_approved: requests?.filter(r => r.status === 'approved').reduce((sum, r) => sum + (r.days_requested || 0), 0) || 0,
