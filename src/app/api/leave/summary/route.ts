...
export async function GET(request: NextRequest) {
  try {
    ...
    const summary = users?.map((member: any) => {
      const userRequests = requests?.filter(r => r.user_id === member.user_id) || []
      const totalLeaveDays = userRequests.reduce((sum, r) => sum + (r.days_requested || 0), 0)
      const usedLeaveDays = userRequests.filter(r => r.status === 'approved').reduce((sum, r) => sum + (r.days_requested || 0), 0)
